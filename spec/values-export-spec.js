...
describe('The legion-capture metrics/values call', function() {
  beforeEach(function() {
    this.port = 4312;
  ...
  it('can generate tabular export of the data', function(done) {
    const x = metrics.sample({
      x: {
        value : 25,
        unit : 'inches',
        interpretation: 'position on x axis'
      },
      y: {
        value : 25,
        unit : 'miles',
        interpretation: 'position on y axis'
      }
    });
    ...
    Promise.all(posts).then(() => {
      const url = this.endpoint + '/metrics/values?' + querystring.stringify({
        project_key: 'my-project-key'
      });
      ...
      expect(json.x.unit).toEqual(['inches']);
